...
// FIX S-2: All report routes now require authentication.
// FIX S-6: sameTenantOnly applied to tenant-scoped settings and downloads.
...
router.post('/settings', authMiddleware, subscriptionGuard, async (req, res) => {
  try {
    ...
    // FIX S-6: Verify requesting user belongs to this tenant
    ...
    const updatedTenant = await Tenant.findByIdAndUpdate(
      tenantId,
      { $set: { reportEmail, weeklyReportDay, monthlyReportDate } },
    ...
    console.log(`✅ [Reports] Settings updated for: ${updatedTenant.companyName}`);
...
router.post('/send-test', authMiddleware, subscriptionGuard, async (req, res) => {
  try {
    ...
    // FIX S-6: Tenant ownership check
    if (!req.user?.isSuperAdmin && req.user?.tenantId?.toString() !== tenantId) {
      return res.status(403).json({ message: 'Forbidden.' });
    }
